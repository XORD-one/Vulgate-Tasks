...
export class Mutahhir extends ethereum.SmartContract {
  static bind(address: Address): Mutahhir {
    return new Mutahhir("Mutahhir", address);
  }
  ...
  _totalTokens(): BigInt {
    let result = super.call("_totalTokens", "_totalTokens():(uint256)", []);

    return result[0].toBigInt();
  }

  try__totalTokens(): ethereum.CallResult<BigInt> {
    let result = super.tryCall("_totalTokens", "_totalTokens():(uint256)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
